...
+ (lsdkm*) instance
{
    ...
    //执行一次初始化
    dispatch_once(&once, ^{
        NSAssert(nil == g_lsdkm, @"g_lsdkm is not nil");
        g_lsdkm = [[lsdkm alloc] init];
    });
...
static int cb_sdk_media(void* p_obj, int protocol, int id, int chnn, int idx, int md_id, l_md_buf_t* p_data)
{
    ...
    if (p_data->ver == L_MD_F_VER)
    {
        ...
        NSLog(@"fmt:%d,type:%d,len:%d", p_data->f_v1.fmt, p_data->f_v1.v_type, p_data->f_v1.len);
        ...
        if (L_FMT_H264 == p_data->f_v1.fmt)
        {
            ...
            NSLog(@"h264 [%x,%x,%x,%x,%x]", p_h264[0], p_h264[1], p_h264[2], p_h264[3], p_h264[4]);
        }
...
static int request_stream(int id, int chnn, int idx)
{
    ...
    NSLog(@"(%s.%d)ret=%d,res=%s",  __FILE__, __LINE__, ret, (NULL != p_res) ? p_res : "error!");
...
static int test_discover()
{
    ...
    for (int i = 0; i < 6; i++ ) {
        ...
        if (NULL != p_devs)
        {
            NSLog(@"discover get devs:%s", p_devs);
            l_sdk_free(p_devs);
        }
        else
        {
            NSLog(@"discover get none!");
        }
    }
...
- (int) test
{
    ...
    // sdk初始化
	int ret = l_sdk_init(NULL);
	NSAssert(0 == ret, @"sdk init error");
    NSLog(@"l_sdk_init ret=%d", ret);
    ...
    NSLog(@"(%s.%d)sdk dec open,ret=%d",__FILE__, __LINE__, ret);
	...
	int id = 0;
	int ret_login = l_sdk_login(&id, req_login);
    NSLog(@"l_sdk_login id=%d, ret_login=%d", id, ret_login);
        ...
        for (int i = 0; i < 200; i++)
        {
            ...
            if (0 == l_sdk_dec_get_md_data(T_STREAM_DEC_ID, &p_md_data))
            {
                ...
                if (NULL != p_md_data)
                {
                    ...
                    NSLog(@"type:%d, wh:[%d.%d], ptr yuv:[0x%p,0x%p,0x%p]", p_md_data->type, p_md_data->w, p_md_data->h,
                                p_md_data->p_y, p_md_data->p_u, p_md_data->p_v);
